...
const FeatureIcon = ({
  ...
  const icons: Record<string, JSX.Element> = {
    globe: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
    ...
    crystal: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
    ...
    shield: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
        <path d="M12 3L4 7V11C4 15.4 7.4 19.5 12 21C16.6 19.5 20 15.4 20 11V7L12 3Z" stroke={color} strokeWidth="1.5" />
        <path d="M9 12L11 14L15 10" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>,
    chart: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
    ...
    bolt: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
        <path d="M13 2L4 14H12L11 22L20 10H12L13 2Z" stroke={color} strokeWidth="1.5" strokeLinejoin="round" />
      </svg>,
    devices: <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] w-full h-[90vh] p-0 bg-background/95 backdrop-blur-xl border-border/50">
        ...
        <div className="relative w-full h-full">
          <button onClick={onClose} className="absolute top-3 right-3 z-20 p-2 rounded-lg bg-muted/80 hover:bg-muted text-foreground transition-colors">
            <X className="w-5 h-5" />
          </button>
...
const LivePreview = ({
  ...
  return <div className="relative bg-muted/20 rounded-xl overflow-hidden h-full min-h-[220px]">
      {/* Top bar with expand button */}
      <div className="absolute top-2 right-2 z-10">
        <button onClick={onExpand} className="p-1.5 rounded-lg bg-muted/80 hover:bg-muted text-foreground/80 hover:text-foreground transition-all backdrop-blur-sm" title="Fullscreen">
          <Maximize2 className="w-3.5 h-3.5" />
        </button>
      </div>
      
      {/* Loading State */}
      {isLoading && showLive && <div className="absolute inset-0 flex items-center justify-center bg-muted/50 z-5">
      ...
      <div className="w-full h-full overflow-hidden relative">
        {imageUrl ?
      // Show uploaded image
      <img src={imageUrl} alt="Project preview" className="w-full h-full object-cover cursor-pointer" onClick={onExpand} /> : showLive && !hasError ? <div className="absolute inset-0 overflow-hidden">
            <iframe src={url} title="Live Preview" className="border-0 pointer-events-none" style={{
          width: '200%',
          height: '200%',
          transform: 'scale(0.5)',
          ...
          top: 0,
          left: 0
        }} loading="lazy" onLoad={() => setIsLoading(false)} onError={handleIframeError} sandbox="allow-scripts allow-same-origin" />
          </div> : <div className="w-full h-full relative group cursor-pointer flex items-center justify-center" style={{
...
const Projects = () => {
  ...
  const projectData = featuredProject || {
    ...
    description: 'A numerology predictions platform supporting English and Hindi with JWT authentication and SEO optimization.',
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6" ref={ref}>
        ...
        duration: 0.5
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold mb-4">
            My <span className="neon-text text-blue-700">Work</span>
          </h2>
        </motion.div>
        ...
        delay: 0.1
            ...
            <div className="flex flex-col md:flex-row gap-4 sm:gap-6">
              ...
              <div className="w-full md:w-[45%] shrink-0">
                <LivePreview isLowEnd={isLowEnd} onExpand={() => handleExpand(projectData.url || 'https://vishwaguru.site')} url={projectData.url || 'https://vishwaguru.site'} imageUrl={projectData.image_url} />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex flex-col gap-3 mb-4">
                  <div>
                    <h3 className="text-lg sm:text-xl md:text-2xl font-heading font-bold mb-1.5">
                      {projectData.title?.includes('.') ? <>
                          {projectData.title.split('.')[0]}<span className="text-primary">.{projectData.title.split('.').slice(1).join('.')}</span>
                        </> : projectData.title}
                    </h3>
                  ...
                  {projectData.url && <a href={projectData.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl font-heading font-semibold text-xs text-primary-foreground bg-gradient-to-r from-primary to-accent hover:shadow-glow-cyan transition-all whitespace-nowrap self-start">
                      Visit Site
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {techStack.map((tech, index) => <motion.span key={tech} initial={{
                  opacity: 0,
                  scale: 0.7,
                  y: 10
                }} animate={isInView ? {
                ...
                transition: {
                  ...
                  damping: 20
                      ...
                      <motion.div className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" whileHover={{
                  rotate: 10,
                  scale: 1.15
                }} transition={{
                  type: "spring",
        ...
        {/* Other Projects Grid */}
        {otherProjects.length > 0 && <motion.div initial={{
        opacity: 0,
        y: 30
      }} animate={isInView ? {
        opacity: 1,
        ...
        delay: 0.3
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {otherProjects.map((project, index) => <motion.div key={project.id} initial={{
            opacity: 0,
            y: 30,
            scale: 0.95
          }} animate={isInView ? {
            ...
            transition: {
              ...
              damping: 20
                  ...
                  <div className="relative h-40 rounded-lg overflow-hidden mb-4 bg-muted/30">
                    {project.image_url ? <img src={project.image_url} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /> : project.url ? <div className="w-full h-full flex items-center justify-center cursor-pointer" style={{
                background: `linear-gradient(135deg, hsl(var(--primary) / 0.2), hsl(var(--accent) / 0.1))`
              }} onClick={() => handleExpand(project.url || '')}>
                        <p className="text-sm font-heading text-foreground/70">Click to preview</p>
                      </div> : <div className="w-full h-full flex items-center justify-center" style={{
                background: `linear-gradient(135deg, hsl(var(--primary) / 0.2), hsl(var(--accent) / 0.1))`
              }}>
                        <p className="text-sm font-heading text-foreground/50">No preview</p>
                      </div>}
                    {project.url && <button onClick={() => handleExpand(project.url || '')} className="absolute top-2 right-2 p-1.5 rounded-lg bg-muted/80 hover:bg-muted text-foreground/80 hover:text-foreground transition-all backdrop-blur-sm opacity-0 group-hover:opacity-100" title="Fullscreen">
                        <Maximize2 className="w-3.5 h-3.5" />
                      </button>}
                  </div>
                  ...
                  {/* Tech Stack */}
                  {project.tech_stack && project.tech_stack.length > 0 && <div className="flex flex-wrap gap-1 mb-3">
                      {project.tech_stack.slice(0, 4).map(tech => <span key={tech} className="px-2 py-0.5 rounded-md text-[10px] font-mono bg-muted/50 text-foreground/80 border border-border/30">
                          {tech}
                        </span>)}
                      {project.tech_stack.length > 4 && <span className="px-2 py-0.5 text-[10px] font-mono text-muted-foreground">
                          +{project.tech_stack.length - 4} more
                        </span>}
                    </div>}

                  {/* Visit Link */}
                  {project.url && <a href={project.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-heading font-semibold text-primary hover:text-primary/80 transition-colors">
                      Visit Site
